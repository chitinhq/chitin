...
#      (one line per run) so the operator can see "what happened
#      when" without parsing journald.
#
# Exit codes:
...
# Structured log helper — one JSON line per call. Extra k=v args
# encoded as additional JSON fields. Stderr mirror so journald
# captures it for systemd units.
emit() {
  local kind="$1" msg="$2"
  shift 2
  local extras=""
  while (($#)); do
    local k="${1%%=*}" v="${1#*=}"
    # Escape backslashes + quotes in values.
    v="${v//\\/\\\\}"
    v="${v//\"/\\\"}"
    extras+=",\"${k}\":\"${v}\""
    shift
...
# Decide: do we need to rebuild?
need_rebuild=0
relevant_changes_since_last="(none)"

if [[ "$old_sha" != "$new_sha" ]]; then
  # New commits exist. Are any kernel-relevant?
  if git diff --quiet "$old_sha" "$new_sha" -- go/ chitin.yaml; then
    # No relevant changes; pull but skip rebuild.
...
# Even with no new commits, the binary may be older than the source
# (operator just rebuilt; binary hasn't been redeployed yet; first run
# of the timer after a manual edit). Force rebuild in that case.
...
# Save prev binary for rollback (if one exists).
if [[ -x "$BIN" ]]; then
  cp -a "$BIN" "$PREV"
fi

# Build. Chitin's go module is nested at go/execution-kernel/go.mod
# (no top-level go.mod), so `go build` needs to run from inside that
# module. cd in a subshell so the rest of the script keeps using
# $REPO as cwd for the smoke-test below.
...
# Smoke-test: a `Task` PreToolUse evaluate must exit 0. With the
# closed-enum normalizer (PR #171), Task → delegate.task →
# default-allow-delegate → exit 0. With the pre-#171 normalizer it
# falls through to default-deny-unknown → exit non-zero. The smoke
# is therefore version-aware: it asserts the kernel knows about
# modern Claude Code tools.
