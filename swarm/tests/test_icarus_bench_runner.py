...
class TestExtractTickMetadata(TestCase):
    ...
    def setUp(self) -> None:
        ...
        _runner["JOBS_DIR"] = Path(self.tmpdir) / "jobs" / "chitin-bench"
    ...
    def test_extracts_steps_from_current_metadata_field(self):
        """Invariant: steps_used is extracted from
        agent_result.metadata.icarus_steps_used when current Harbor
        artifacts omit top-level step counters."""
        self._write_trial_result("icarus-meta-steps", "trial-0", {
            "agent_result": {
                "metadata": {
                    "icarus_block_reason": "step_budget_exceeded",
                    "icarus_steps_used": 30,
                },
            },
            "verifier_result": {"rewards": {"reward": 0.0}},
        })
        result = {"status": "ran", "job_name": "icarus-meta-steps"}
        meta = _runner["_extract_tick_metadata"](result, "icarus-meta-steps")
    ...
    def test_extracts_steps_from_chitin_bench_metadata_fields(self):
        """Invariant: current chitin-bench artifacts are parsed from
        chitin_bench_* metadata keys."""
