"""Misroute test infrastructure — shared helpers, schemas, and constants.

This module provides the test harness required by test_misroute.py and
downstream misroute test files. All fixtures, channel IDs, DB schemas,
and helper functions live here, separate from conftest.py (which only
holds pytest fixtures).
...
# Self-contained copy for fixture use — hermes_cli is not required.
# If agent-bus is importable, the canonical version is preferred via
...
def assert_temp_path(path: Path) -> Path:
...
def wait_for_message(
    ...
    *,
    ...
    Uses SQLite's update_hook for deterministic waiting when available
    (Python 3.12+), falling back to tight-poll mode for older builds.
    NOT sleep-based — the poll interval is a tight 20ms fallback only.
    ...
    # Use update_hook for instant notification when the DB changes.
...
def query_gateway_logs_by_message(
    gateway_conn: sqlite3.Connection,
    message_id: int,
) -> list[sqlite3.Row]:
    """Retrieve ALL gateway log entries for a given message_id.

    Unlike query_gateway_logs (which requires channel_id), this returns
    entries across all channels — useful for verifying a message only
    appears in expected channels and nowhere else.
...
# ── DB path helpers (used by conftest.py fixtures) ─────────────────────

def make_bus_db(tmp_path: Path) -> sqlite3.Connection:
    """Create a fresh agent-bus DB under tmp_path with safety checks."""
    db_path = assert_temp_path(tmp_path / "test_bus.db")
...
def make_gateway_db(tmp_path: Path) -> sqlite3.Connection:
    """Create a fresh gateway-log DB under tmp_path with safety checks."""
    db_path = assert_temp_path(tmp_path / "test_gateway.db")
    conn = sqlite3.connect(str(db_path))
    conn.row_factory = sqlite3.Row
