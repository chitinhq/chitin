#!/usr/bin/env bash
# install-minnie.sh — wire `minnie` and `octi-worker` onto PATH and verify
# kitty remote control. Idempotent per constitution §6.
#
# What it does:
#   1. Symlinks swarm/bin/minnie and swarm/bin/octi-worker into ~/.local/bin/
...
for tool in minnie octi-worker; do
...
if ! command -v kitty >/dev/null 2>&1; then
  echo "WARNING: kitty terminal not on PATH; install kitty before using minnie." >&2
  exit 0
fi
...
Then restart kitty. minnie will not work until this succeeds.
EOF
  exit 2
fi
...
if [[ -z "${OCTI_DISCORD_WEBHOOK_URL:-}" ]]; then
  ...
  echo "  \`minnie watch\` requires a webhook URL."
fi

echo "install complete. Try: minnie --help"
