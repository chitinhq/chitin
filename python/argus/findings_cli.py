...
def cmd_findings(args) -> int:
    ...
    if not db_path.exists():
        print(f"argus: index not found at {db_path}", file=sys.stderr)
        return 1
    try:
...
def cmd_finding_action(args) -> int:
    ...
    conn.close()
    if not ok:
        print(f"argus: no finding with id={args.finding_id}", file=sys.stderr)
        return 3
    print(f"argus: finding {args.finding_id} -> {args.action}", file=sys.stderr)
    return 0
