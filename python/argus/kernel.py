...
from argus import config, findings_store, gpu, llm, migrations, prompts
from argus.detectors import run_all_detectors
from argus.judge import judge
...
def run(
    *,
    ...
    if not Path(db_path).exists():
        print(f"argus: index not found at {db_path}; run 'argus index' first", file=sys.stderr)
        return 1
    ...
    if integrity != "ok":
        print(f"argus: integrity_check returned {integrity!r}; refusing to start", file=sys.stderr)
        return 2
    applied = migrations.apply_pending(conn)
    if applied:
        print(f"argus: applied schema migrations: {applied}", file=sys.stderr)
