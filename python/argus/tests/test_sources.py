...
from argus.sources import _open_kanban_readonly, _respect_gh_rate_limit, ingest_git_repo, ingest_kanban_sources


def _argus_conn(tmpdir: str) -> sqlite3.Connection:
    db_path = Path(tmpdir) / "argus.db"
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        def fake_runner(args, **_kwargs):
            ...
            if args[:4] == ["gh", "pr", "view", "14"]:
                return Result(0, json.dumps({
                    "files": [{"path": "python/argus/detectors.py"}],
                    "reviews": [],
