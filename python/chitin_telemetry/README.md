...
Argus is shipped as a Python package in `python/chitin_telemetry/` inside the chitin
repo. The recommended install uses `uv` to expose `chitin-telemetry` as a console
script in `~/.local/bin/`:

```bash
# From repo root
uv tool install ./python/chitin_telemetry
...
```bash
cp python/chitin_telemetry/systemd/*.service ~/.config/systemd/user/
cp python/chitin_telemetry/systemd/*.timer  ~/.config/systemd/user/
...
python -m chitin_telemetry index --decisions-dir ~/.chitin

# Generate daily report
python -m chitin_telemetry report --report-dir ~/.chitin/reports

# Query the index with natural language
python -m chitin_telemetry query "How many denies by rule_id in the last 24h?"
```
...
python -m chitin_telemetry query "Show deny count grouped by agent"

# What are the most common deny rules?
python -m chitin_telemetry query "List top 10 deny rules by count"

# When was the last activity for each agent?
python -m chitin_telemetry query "Show each agent's last event timestamp"
```
...
```bash
# Run all tests
python -m pytest python/chitin_telemetry/tests/ -v

# Run specific test suite
python -m pytest python/chitin_telemetry/tests/test_indexer.py -v
python -m pytest python/chitin_telemetry/tests/test_detectors.py -v
python -m pytest python/chitin_telemetry/tests/test_reporter.py -v
```
