"""CLI entry: chitin telemetry command."""
...
from chitin_telemetry import beliefs, findings_cli, migrations
from chitin_telemetry.indexer import init_db
from chitin_telemetry.logs import follow_all_sources, index_all_sources
from chitin_telemetry.reporter import generate_daily_report
from chitin_telemetry.sources import ingest_git_sources, ingest_kanban_sources
...
def cmd_report(args) -> int:
    ...
    if not db_path.exists():
        ...
        print("Run 'chitin-telemetry index' first.", file=sys.stderr)
        return 1
...
def cmd_ingest_beliefs(args) -> int:
    ...
    for alert in result.alerts:
        print(f"chitin-telemetry: {alert.adapter}: {alert.message}", file=sys.stderr)
    print(
...
def cmd_query(args) -> int:
    ...
    from chitin_telemetry import llm, prompts
    ...
    if not result.ok or not result.text:
        print(f"chitin-telemetry: LLM unavailable ({result.error or result.skipped_reason})", file=sys.stderr)
        return 2

    sql = _sanitize_readonly_select(result.text)
    if not sql:
        print("chitin-telemetry: LLM returned no safe read-only SELECT", file=sys.stderr)
    ...
    try:
        ...
        print(f"chitin-telemetry: query error: {e}", file=sys.stderr)
...
def cmd_kernel(args) -> int:
    from chitin_telemetry import kernel
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="chitin_telemetry", description="Observatory: tail, index, detect, report.")
