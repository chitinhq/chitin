...
- **`<think>` leak fixed.** `chitin_telemetry.llm.strip_thinking()` handles three formats:
...
- **`chitin_telemetry.llm`** consolidates all qwen calls through ollama's HTTP API
...
- **`chitin_telemetry.judge`** verifies every LLM output before it lands in a report
...
- **`chitin_telemetry.prompts.wrap_untrusted()`** is the single chokepoint for
...
1. Install the unit:
   ```bash
   cp python/chitin_telemetry/systemd/argus-kernel.service ~/.config/systemd/user/
...
3. Optional: edit `~/.argus/config.yaml` to tune cadence + thresholds.
   See `python/chitin_telemetry/config.py` for the full schema.
