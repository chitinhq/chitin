"""CLI entry: python -m chitin_telemetry.telemetry ...

Telemetry consumes gov-decisions JSONL, mines repeat deny patterns, and emits
bounded invariant proposals for `chitin.yaml`. It reuses the canonical
decisions loader, detector, and rule templates; the telemetry-specific contract
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="chitin_telemetry.telemetry",
    ...
    p.add_argument("--out-dir", default="python/chitin_telemetry/out", help="Output directory")
    p.add_argument(
        "--decisions-dir",
...
def _proposal_for_finding(finding, *, threshold: int, now: datetime) -> dict[str, Any] | None:
    ...
    proposal = DispatchPolicyUpdate(
        ...
        attribution=Attribution(
            ...
            sentinel_source=f"chitin_telemetry.telemetry:{now.date().isoformat()}",
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    json_path = out_dir / f"telemetry-{date_str}.json"
    md_path = out_dir / f"telemetry-{date_str}.md"
