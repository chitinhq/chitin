...
def test_llm_failure_falls_back_to_heuristic():
    with patch("chitin_telemetry.llm_draft._call_ollama", side_effect=RuntimeError("boom")):
...
def test_llm_success_returns_llm_draft():
    ...
    with patch("chitin_telemetry.llm_draft._call_ollama", side_effect=fake_call):
        out = enrich_with_llm([(_pattern(), _heuristic_draft())])
    assert len(out) == 1
...
def test_no_heuristic_passes_through():
    with patch("chitin_telemetry.llm_draft._call_ollama", return_value="..."):
