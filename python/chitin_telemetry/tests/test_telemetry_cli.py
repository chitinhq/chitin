...
def test_cli_runs_end_to_end_on_fixture(tmp_path, fixtures_dir):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "chitin_telemetry.decisions",
         "--window", "100d",
         "--top-n", "10",
         "--out-dir", str(out_dir),
...
def test_telemetry_cli_outputs_candidate_invariant_proposals(tmp_path, fixtures_dir):
    decisions_dir = tmp_path / "decisions"
    decisions_dir.mkdir()
    _stage_fixture(decisions_dir, fixtures_dir)
    out_dir = tmp_path / "out"
    ...
    result = subprocess.run(
        [sys.executable, "-m", "chitin_telemetry.telemetry",
         "--window", "100d",
         "--top-n", "5",
         "--out-dir", str(out_dir),
         "--decisions-dir", str(decisions_dir),
...
def test_telemetry_empty_boundary_writes_no_candidate_proposals(tmp_path, fixtures_dir):
    decisions_dir = tmp_path / "decisions"
    decisions_dir.mkdir()
    _stage_fixture(decisions_dir, fixtures_dir)
    out_dir = tmp_path / "out"
    ...
    body = json.loads(list(out_dir.glob("telemetry-*.json"))[0].read_text())
    assert body["patterns"] == []
    assert body["metadata"]["promotion"]["proposal_count"] == 0
    assert body["metadata"]["promotion"]["status"] == "no-candidate"


def test_telemetry_max_boundary_limits_patterns_to_top_n(tmp_path, fixtures_dir):
    decisions_dir = tmp_path / "decisions"
    decisions_dir.mkdir()
    _stage_fixture(decisions_dir, fixtures_dir)
    out_dir = tmp_path / "out"
...
def test_telemetry_error_boundary_rejects_missing_decisions_dir(tmp_path):
    result = subprocess.run(
        [sys.executable, "-m", "chitin_telemetry.telemetry",
         "--window", "7d",
         "--out-dir", str(tmp_path / "out"),
...
def test_telemetry_threshold_above_enters_review_queue(tmp_path):
    decisions_dir = tmp_path / "decisions"
    decisions_dir.mkdir()
...
def test_telemetry_threshold_below_stays_out_of_review_queue(tmp_path):
    decisions_dir = tmp_path / "decisions"
    decisions_dir.mkdir()
...
def test_telemetry_threshold_from_config_clamps_below_floor(tmp_path):
    decisions_dir = tmp_path / "decisions"
    decisions_dir.mkdir()
    _stage_repeated_deny_fixture(decisions_dir, 2)
