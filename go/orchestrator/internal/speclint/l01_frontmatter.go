...
// frontmatter is missing or unterminated, it returns ("", 0, err) (or
// (_, 1, err) once the opening fence has been recognised).
func extractFrontmatter(content string) (string, int, error) {
	lines := strings.Split(content, "\n")
	if len(lines) == 0 || strings.TrimRight(lines[0], "\r") != "---" {
		return "", 0, errors.New("missing — file must start with '---' fence")
	}
...
// or nil when the field is well-formed. The yaml.Node.Line value is
// 1-based relative to the YAML body we fed the parser; we add baseLine
// (the line of the opening "---") so violations point at the line in the
// source spec.md, not the line in the extracted YAML body.
func validateL01Field(specPath, key string, node *yaml.Node, baseLine int) *Violation {
	...
	case "depends_on", "related":
		// A bare "depends_on:" with no value parses as a null scalar, not a
		// sequence — that is ambiguous (null vs empty list) and rejected so
		// authors write the empty list explicitly as "[]".
