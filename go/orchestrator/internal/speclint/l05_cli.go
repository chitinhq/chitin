...
// and returns []Violation. The shared Violation / Severity types and the
...
// Severity, SeverityError, SeverityWarning, and Violation live in lint.go
// (canonical home for the shared types). L05 used to declare its own
// `Finding` type alongside duplicates of the shared Severity enum; both
// have been folded into the canonical Violation/Severity so every rule
...
//
// Returns one Violation per violation. Each finding has Rule="L05" and
// Severity=SeverityError. Returns nil if both files are empty.
func L05CLISurface(specFile, specMD, tasksFile, tasksMD string, allowlist []AllowlistEntry) []Violation {
...
func checkGhAPIPaths(file, text string) []Violation {
	if text == "" {
		return nil
	}
	var findings []Violation
...
func checkChitinSubcommands(file, text string, allowed, introduced map[string]bool) []Violation {
	if text == "" {
		return nil
	}
	var findings []Violation
...
func scanBinarySubs(file, text, binary string, re *regexp.Regexp, allowed, introduced map[string]bool) []Violation {
	var findings []Violation
