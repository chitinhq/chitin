...
//
//   - types.go     — shared Entry row shape rendered by the formatters
//   - format_md.go — markdown table renderer (FR-006, T006)
//
// Planned in sibling PRs (spec 114 tasks T002–T005, T007):
//
...
//
// The shared Entry type (types.go) is the single shape the formatters
// render. It carries the PR identity plus the canonical reason kind
// from the FR-008 closed taxonomy.
package queue
