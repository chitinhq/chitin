...
//   - design_judgement_required        ← spec_iteration_escalated  [spec 115 FR-010]
//   - lint_violation_unresolvable      ← spec_iteration_escalated  [spec 115 FR-010]
//
...
//
// Spec 115 FR-009 adds `spec_iteration_escalated` — the spec-PR sibling
// of `pr_iteration_escalated`. Both share the (pr_number, reason)
// payload shape, so the scanner treats them as a single chain-derived
// surface and the operator queue (filter.go) consumes them uniformly.
...
// specIterEscalatedReasons is the closed set of `spec_iteration_escalated`
// payload reasons declared in spec 115 FR-010. It re-uses the three
// reasons spec 113 also emits (iteration_cap_hit / human_reviewer_present
// / lease_lost — the same operator-action semantics apply on spec PRs)
// AND adds two spec-PR-specific kinds:
//
//   - design_judgement_required   — FR-008 classifier fired; operator
//     judgement needed because the comment is not mechanical
//   - lint_violation_unresolvable — driver couldn't fix the linter
//     violation and didn't justify patching the allowlist
//
// An event whose reason is outside this set is skipped, mirroring the
// pr_iteration_escalated handling.
var specIterEscalatedReasons = map[string]struct{}{
	"iteration_cap_hit":           {},
	"human_reviewer_present":      {},
	"lease_lost":                  {},
...
func classifyReason(eventType, payloadReason string) (string, bool) {
	...
	case "spec_iteration_escalated":
		if _, ok := specIterEscalatedReasons[payloadReason]; !ok {
			return "", false
		}
		return payloadReason, true
	}
