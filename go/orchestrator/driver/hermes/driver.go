...
// Driver wraps the Hermes agent CLI behind the spec-075 AgentDriver contract.
type Driver struct {
	command string
}
...
// New returns a Hermes AgentDriver.
func New(opts ...Option) *Driver {
	d := &Driver{command: "hermes"}
...
// Card returns the Hermes capability card.
...
// Ready reports whether the Hermes CLI binary is available.
func (d *Driver) Ready(ctx context.Context) (bool, string) {
	if err := ctx.Err(); err != nil {
		return false, err.Error()
	}
	if _, err := exec.LookPath(d.command); err != nil {
		return false, fmt.Sprintf("Hermes runtime %q not found: %v", d.command, err)
	}
	return true, ""
}

// Invoke shells out to Hermes in the work unit's dedicated worktree.
func (d *Driver) Invoke(ctx context.Context, wu driver.WorkUnit) (driver.Result, error) {
	...
	prompt := promptFor(wu)
	cmd := exec.CommandContext(ctx, d.command, "chat", "-q", prompt)
