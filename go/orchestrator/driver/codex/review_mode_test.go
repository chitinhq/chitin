...
// TestInvoke_ReviewMode_CleanJSONEmitsValidatedVerdict covers spec 110 FR-006
// (parity with spec 109's claudecode test): when the codex review-mode
// invocation receives a clean StructuredVerdict JSON document on stdout, the
// driver emits StatusSucceeded with the validated (canonically re-serialized)
...
//
// This is the companion test to the claudecode case that guards the
// 2026-05-24 dogfood failure where codex returned prose and the activity
// classified the outcome as FailureMalformedJSON, blocking verdict
// aggregation on PR #1007. With T001-T003 in place, a clean CLI response is
// propagated to the activity in the spec-compliant shape.
//
...
// and what claudecode's review_mode_test.go uses to trigger the review-mode
// codepath.
