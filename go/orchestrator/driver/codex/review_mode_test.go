...
// there would silently disable a real safety boundary.
//
// Discriminator: review-mode is keyed on SpecID="094" / TaskID="review"
// (mirroring activities/review/dispatch_machine_reviewer.go and the
// claudecode T003/spec-109 wiring). This work unit is an ordinary
// implementation task (SpecID="999", TaskID="T001"), so it must route
// through the plain Invoke path — no `--skip-git-repo-check` in argv.
