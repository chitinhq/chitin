...
// Driver wraps the Gemini CLI behind the spec-075 AgentDriver contract
// (spec 080 US1). Like the other hosted CLI drivers it is a stateless,
// headless one-shot per work unit — no persistent agent process.
type Driver struct {
	command string
}
...
// New returns a Gemini AgentDriver.
func New(opts ...Option) *Driver {
	d := &Driver{command: "gemini"}
...
// Card returns the Gemini capability card. Gemini's distinguishing trait is
// its very large context window; capabilities are scoped to the coding and
// research work the runtime genuinely handles.
...
// Ready reports whether the Gemini CLI binary is available.
func (d *Driver) Ready(ctx context.Context) (bool, string) {
	if err := ctx.Err(); err != nil {
		return false, err.Error()
	}
	if _, err := exec.LookPath(d.command); err != nil {
		return false, fmt.Sprintf("Gemini runtime %q not found: %v", d.command, err)
	}
	return true, ""
}

// Invoke shells out to the Gemini CLI in the work unit's dedicated worktree.
// `-p` runs the CLI headless; `--approval-mode yolo` lets the agent apply its
// edits without an interactive prompt — the work unit is an isolated worktree
// and the chitin kernel governs every tool call regardless.
func (d *Driver) Invoke(ctx context.Context, wu driver.WorkUnit) (driver.Result, error) {
	...
	prompt := promptFor(wu)
	cmd := exec.CommandContext(ctx, d.command, "-p", prompt, "--approval-mode", "yolo")
