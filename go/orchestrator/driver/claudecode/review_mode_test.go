...
// TestInvoke_ReviewMode_ValidationFailureEmitsMalformedVerdict covers spec 109
// T007 / FR-005 / US2: when the claudecode review-mode invocation returns a
// JSON document that parses cleanly but violates one of the spec 094 FR-014
// per-enum invariants — here `verdict="approve"` with a non-empty `blockers`
// list — the driver MUST surface the failure as
// Result{Status: StatusFailed, Explanation: "malformed_verdict: ..."} with
// the specific validation invariant name (from invariants.go) embedded so
// downstream consumers (and post-mortem readers in Temporal history) can
// distinguish "approve blockers must be empty" from a JSON parse error or
// a missing-JSON failure.
//
// This guards against a regression where Validate() runs but its returned
// error is dropped or replaced by a generic "invalid verdict" message —
// either case would erase the diagnostic value of FR-014's named invariants.
// The chosen probe (approve with blockers) is the simplest possible invariant
// violation: structurally valid JSON, valid enum, single rule violated.
func TestInvoke_ReviewMode_ValidationFailureEmitsMalformedVerdict(t *testing.T) {
	dir := t.TempDir()
	binPath := filepath.Join(dir, "claude")
	// FR-014 invariant 1: approve ⇒ blockers MUST be empty. This JSON is
	// well-formed and the enum is valid; only the per-enum rule fails.
	invalidJSON := `{"verdict":"approve","concerns":[],"recommendations":[],"blockers":["this should not be here"]}`
	script := "#!/usr/bin/env bash\n" +
		"cat <<'JSON'\n" + invalidJSON + "\nJSON\n" +
		"exit 0\n"
	if err := os.WriteFile(binPath, []byte(script), 0o755); err != nil {
	...
	// The named invariant from verdict.invariants.go must survive into the
	// explanation so an operator reading Temporal history sees *which* rule
	// failed, not just that something did.
	if !strings.Contains(res.Explanation, "approve_blockers_must_be_empty") {
		t.Errorf("explanation missing named invariant 'approve_blockers_must_be_empty'; got %q", res.Explanation)
	}
	// The raw model output should also be appended (per malformedVerdictExplanation)
	// so the post-mortem reader can see what the model actually emitted.
	if !strings.Contains(res.Explanation, `"verdict":"approve"`) {
		t.Errorf("explanation missing raw model output; got %q", res.Explanation)
	}

	// Negative control: the explanation must NOT be a parseable StructuredVerdict
	// (which would mean reviewResult took the success path despite the validation
	// failure).
	var stray verdict.StructuredVerdict
	if err := json.Unmarshal([]byte(res.Explanation), &stray); err == nil {
		t.Errorf("explanation should not parse as StructuredVerdict on validation failure; got %q", res.Explanation)
	}
}
