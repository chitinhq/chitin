...
// spec 094 StructuredVerdict JSON contract the model must emit. It is
// inlined in the review-mode prompt so the model can self-reference the
// authoritative schema when it needs to disambiguate edge cases (FR-002).
...
// required by spec 109 FR-001 / FR-002.
...
// {...} block in the input. Per spec 109 FR-003 (c), callers receive the
// original raw string alongside this error and surface it to the verdict
...
// stdout. Per spec 109 FR-003, applies in order:
//
