...
// Driver wraps the OpenClaw agent (an ACP runtime) behind the spec-075
...
// New returns an OpenClaw AgentDriver.
func New(opts ...Option) *Driver {
	d := &Driver{command: "openclaw"}
...
// Ready reports whether the OpenClaw CLI binary is available.
func (d *Driver) Ready(ctx context.Context) (bool, string) {
	if err := ctx.Err(); err != nil {
		return false, err.Error()
	}
	if _, err := exec.LookPath(d.command); err != nil {
		return false, fmt.Sprintf("OpenClaw runtime %q not found: %v", d.command, err)
	}
	return true, ""
}

// Invoke shells out to OpenClaw in the work unit's dedicated worktree.
func (d *Driver) Invoke(ctx context.Context, wu driver.WorkUnit) (driver.Result, error) {
	...
	prompt := promptFor(wu)
	cmd := exec.CommandContext(ctx, d.command, "agent", "-p", prompt)
