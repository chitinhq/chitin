...
func (d *Driver) Invoke(ctx context.Context, wu driver.WorkUnit) (driver.Result, error) {
	...
	if err != nil && strings.Contains(errOut, "launch") && strings.Contains(errOut, "unknown") {
		errOut += "; ollama v0.21+ required for launch subcommand"
	}
	res := claudecodeshared.ResultFromCommandWithStore(ctx, d.blobs, wu, d.ID(), out, errOut, err)
	return res, nil
}
