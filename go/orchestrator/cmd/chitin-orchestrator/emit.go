...
type SchedulerStartedPayload struct {
	...
	// DriverID is the selected driver for a single-driver dispatch shape such
	// as whole-spec mode. Omitted for older/per-task events where selection is
	// per node and only known inside the scheduler workflow.
