...
## Nx generators that spawn drivers — sync LLM-completion (filed 2026-05-04)

Today's chitin pattern: drivers run async via Temporal (long-lived,
backlog-entry-shaped). Nx generators today are pure deterministic
templates (file scaffolding, config edits). The bridge worth
building: an Nx generator that synchronously spawns a chitin driver
mid-generation, gated through the same governance kernel.

Why: generators are great for boilerplate but boilerplate often
has pieces that need context-aware judgment ("name this thing for
what it actually does"). Drivers are great for judgment but
sometimes need a clear starting structure. Combined unifies
deterministic-shape + LLM-judgment under chitin's gov.

See project memory `project_nx_generators_spawn_drivers.md` for the
full vision.

### `nx-generator-with-sync-driver-poc`
...
POC for the synchronous-driver-from-Nx-generator pattern. Pick one
narrow use case: `nx generate @chitin:investigate <issue-or-pr>`.
The generator:
- Reads the issue/PR via gh API
- Reads relevant chain events for the affected component
- Synchronously spawns an analyst-role driver (default:
  openclaw-glm-flash, env-overridable) with the gathered context
- Awaits the driver's structured output
- Writes a markdown investigation doc at
  `docs/observations/investigate-<id>-<YYYY-MM-DD>.md`

The driver's tool calls go through the same chitin-governance plugin
that gates the autonomous swarm — synchronous dispatch doesn't
bypass gov; it just doesn't enqueue Temporal.

**Acceptance:**
- [ ] `pnpm exec nx generate @chitin:investigate 285` produces a
      markdown report with: PR summary, affected files, related
      chain events, an analyst-driver paragraph on what to watch
- [ ] Failure modes (timeout, denial, env unset) produce clear errors
      and exit non-zero
- [ ] Cost is bounded: max 1 driver dispatch per generator run, with
      a wall-clock cap (default 60s)
- [ ] No drift from chitin-governance: every tool call the driver
      makes shows up in gov-decisions, same as a Temporal-dispatched
      activity

### `nx-generator-driver-helper`

```yaml
id: nx-generator-driver-helper
tier: T2
status: in_design
estimated_loc: 200
...
Phase 2 of the Nx-generators-spawn-drivers vision. After the POC
proves the sync-await pattern works, factor out a reusable helper
so future generators don't reimplement:
`runDriverSynchronously(req: ExecutionRequest, opts) → DriverResult`.

Same ExecutionRequest schema as the Temporal path; the helper
decides whether to enqueue (long-running, audit-trail) or
spawn-await (interactive, generator). Both routes go through
chitin-governance.

**Acceptance:**
- [ ] Library `@chitin/nx-driver-runner` exports
