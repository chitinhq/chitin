...
- [X] T005 [P] Add a regression test in `tasks_lint_test.go` that asserts `tasks-lint` and `runSchedule`'s DAG validation agree on classification for the same fixture spec — the matcher must be the same codepath (SC-002)
