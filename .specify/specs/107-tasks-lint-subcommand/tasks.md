...
- [X] T004 [US2] Add an integration test in `tasks_lint_test.go` for `--json` output — same fixture as T003, assert `--json` flag produces a parseable JSON array with one object per task, `classified` field is true|false per task
...
- [ ] T006 Update CHANGELOG entry under `## Unreleased` in `apps/chitin-console-api/CHANGELOG.md` mentioning the new `tasks-lint` operator subcommand and its spec 107 reference
