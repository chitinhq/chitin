...
## Lane spec (the 5 skills Icarus owns in v1)

| Skill | Example tickets | Post-check |
|---|---|---|
| **mechanical** | rename symbols across N files, bulk import-sort, dead-code prune | git diff applies cleanly + tests still pass |
| **lint-fix** | run linter, propose fixes, write fixed file | `lint` exit 0 on fixed file |
| **log-pattern** | parse a log file, surface top-N anomaly patterns + counts | output is valid JSON matching schema |
| **doc-from-code** | generate README section from module exports | renders as valid markdown, no broken refs |
| **triage-classify** | given ticket title + body, suggest owner (red/Ares/Clawta/icarus) + skill label | output is a valid (owner, skill) tuple from the enum |
...
3. **N=2 retry cap.** If post-check fails twice, escalate to Clawta with partial work + `block_reason=local_ceiling_exceeded`. No infinite-retry hiding silent failures.
4. **VRAM-shared with argus.** Icarus uses the same loaded ollama model when possible. If argus's hot model differs from Icarus's preferred model, Icarus warm-loads its preferred model and accepts argus cold-start on next tick. NO concurrent second large-model load.
5. **WORKER_RECEIPT required.** Every Icarus action (success OR loud-fail) posts a WORKER_RECEIPT to swarm board + #swarm thread 9 via agent-bus. Same contract as red / Ares / Clawta per swarm redesign.
...
1. **Week 1 (after operator ratify):** Stub icarus-watcher with 1 lane (lint-fix), wire to swarm board, run for 5 days with operator-curated tickets. Measure post-check pass rate + escalation rate. **Gate: ≥70% pass rate or kill the project.**
2. **Week 2:** If Week-1 gate passed, add log-pattern + triage-classify lanes. Run for 5 days. Re-measure.
3. **Week 3:** Add mechanical + doc-from-code if Week-2 succeeded. Ratify Icarus as default for the 5 lanes.
4. **Decommission criteria:** If after Week 1, post-check pass rate <50% on the easiest lane, kill the project. Not worth the cognitive overhead of a 4th driver if it can't reliably ship.
