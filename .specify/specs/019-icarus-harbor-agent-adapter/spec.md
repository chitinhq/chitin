# 019 — IcarusHarborAgent Adapter: Thin Baseline (terminal-bench@2.0)

> **Spec:** 019-icarus-harbor-agent-adapter
> **Author:** red
...
**NOTE: Originally numbered 036 in the ticket, then renumbered to 038, and now renumbered to 019 to resolve collision with 038-octi-persistent-claude-session (the canonical spec 038). The directory is .specify/specs/019-icarus-harbor-agent-adapter/. 
...
**INV-019-HA-1: No speculative deterministic router.**
...
- `.specify/specs/019-icarus-harbor-agent-adapter/**` — this spec directory
...
- [ ] **AC-2:** No deterministic code paths execute during baseline runs. Every agent action goes through the LLM. (INV-019-HA-1)
...
1. **INV-019-HA-1:** No speculative deterministic router. Every turn goes through the LLM. (See §5.)
2. **INV-019-HA-2:** Loud-fail taxonomy preserved verbatim from IcarusAgent. New additions (`tmux_error`, `harbor_setup_error`) require explicit Clawta sign-off per msg 5408 amendment.
3. **INV-019-HA-3:** Temperature=0, seed=0. Two runs of the same task with identical bootstrap produce comparable (not byte-identical) results.
4. **INV-019-HA-4:** Step budget, wallclock, and loop detection are safety rails, not deterministic layers. They stop the agent from burning infinite compute; they do not make agent decisions.
...
This spec was originally numbered 036 in the ticket, then renumbered to 038, and now renumbered to **019** to resolve the collision with `038-octi-persistent-claude-session` (the canonical spec 038, parent of the Octi ecosystem).
...
- 038 was used but collides with `038-octi-persistent-claude-session`
- The spec-kit entry now uses 019, the first available number.

The spec-kit directory was originally created on branch `agent/036-icarus-harbor-agent-adapter-spec` and later at `038-icarus-harbor-agent-adapter`. It now lives at `019-icarus-harbor-agent-adapter`.
