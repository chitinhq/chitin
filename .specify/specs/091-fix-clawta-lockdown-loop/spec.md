...
- **FR-007** *(Ares review — stop-hook reentrancy)*: When the governance gate returns `continue:false`, the driver harness MUST stop the current continuation loop and MUST NOT re-enter a stop hook, retry hook, or forced-continuation path that can produce another gated tool call for the same denied action. This is the canonical fix pattern from Anthropic's [Claude Code issue #55754](https://github.com/anthropics/claude-code/issues/55754) and the deep-research report at `docs/strategy/chitin-orchestrator-industry-alignment-2026-05-22.md`.
- **FR-008** *(Ares review — reentrancy marker)*: The harness MUST record a `stop_hook_active` / equivalent reentrancy marker while handling a forced stop. If the marker is already active when a new continuation block would fire, the harness MUST exit rather than invoking another continuation. This is the published Anthropic fix for the exact failure mode.
- **FR-009** *(Ares review — N-bounded forced continuations)*: After N forced continuations for the same work unit (bound configurable, suggested N=3), the orchestrator MUST mark the work unit failed/blocked and MUST emit telemetry (chain event `event_type: "stop_signal_ignored"` or equivalent) instead of continuing the loop. The bound is a hard cap; the harness cannot opt out.
