...
function makeGateInputs(overrides: Partial<GateInputs> = {}): GateInputs {
  return {
    ...
    pr_files: ['apps/runner/src/foo.ts'],
    entry_file_scope: 'apps/runner/src/foo.ts',
    ci_green: true,
    ...overrides,
  };
}
...
describe('evaluateGates', () => {
  ...
  it('fails when diff touches a T5-shape path (chitin.yaml)', () => {
    const r = evaluateGates(
      makeGateInputs({
        pr_files: ['chitin.yaml', 'apps/runner/src/foo.ts'],
      }),
    );
    expect(r.passed).toBe(false);
  ...
  it('passes when diff is in a subdirectory of the scope file (directory-style)', () => {
    const r = evaluateGates(
      makeGateInputs({
        pr_files: ['apps/runner/src/lib/util.ts'],
        entry_file_scope: 'apps/runner/src/',
      }),
    );
    expect(r.passed).toBe(true);
  });
