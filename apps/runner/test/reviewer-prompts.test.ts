...
describe('parseReviewerOutput', () => {
  ...
  it('roundtrips findings with line numbers', () => {
    const tail = `${REVIEW_MARKER}{"decision":"request_changes","confidence":"high","findings":[{"severity":"🔴","file":"apps/runner/src/dispatcher.ts","line":372,"category":"bug","summary":"writeDispatchMarker fires before submit","suggested_fix":"reorder"}]}`;
    const r = parseReviewerOutput(tail);
    expect(r.ok).toBe(true);
    if (r.ok) {
...
describe('buildAdversarialReviewerPrompt', () => {
  ...
  it('renders the file: scope when provided', () => {
    const out = buildAdversarialReviewerPrompt({
      ...baseInputs,
      entry_file_scope: 'apps/runner/src/foo.ts, libs/contracts/src/bar.ts',
    });
    expect(out).toContain('apps/runner/src/foo.ts');
